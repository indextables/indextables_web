...
```scala
libraryDependencies += "io.indextables" % "indextables_spark" % "0.5.4_spark_3.5.3" classifier "linux-x86_64-shaded"
```

## Spark Shell

```bash
spark-shell --packages io.indextables:indextables_spark:0.5.4_spark_3.5.3:linux-x86_64-shaded
```
...
1. Download the shaded JAR from Maven Central:
   ```
   https://repo1.maven.org/maven2/io/indextables/indextables_spark/0.5.4_spark_3.5.3/indextables_spark-0.5.4_spark_3.5.3-linux-x86_64-shaded.jar
   ```
...
```bash
#!/bin/sh
cp /Volumes/my_catalog/my_schema/artifacts/indextables_spark-0.5.4_spark_3.5.3-linux-x86_64-shaded.jar /databricks/jars
```
